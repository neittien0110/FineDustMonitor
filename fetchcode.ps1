...
#--------------------------------------------------------------------------------
# Ép Console hiển thị tiếng Việt (UTF-8)
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
# Ép PowerShell xử lý luồng dữ liệu theo chuẩn UTF-8
...
$line  = $cmdOutput | Select-String -Pattern 'Chip type:\s+(.+)'
# Địa chỉ firmware_address của code
$firmware_address = 0
...
if ($line ) {
    ...
    if ($chipInfo -match "esp32-c3"){
        ...
        $firmware_address = 0x10000
    } elseif ($chipInfo -match "esp8266"){
        $chipInfo="esp8266"
        $BUILD_PATH=".\.pio\build\d1_mini"
        $firmware_address = 0x0000
    }
} else {
...
python -m esptool --chip $chipInfo `
    ...
    write-flash -z `
    $firmware_address  $BUILD_PATH\firmware.bin
